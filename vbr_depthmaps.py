...
import torch
import depth_pro  # Import the depth_pro library you used before

from vbr_dataset import vbrInterpolatedDataset, get_paths_from_scene, load_calibration


def parse_args():
    parser = argparse.ArgumentParser(description="Batch depth map estimation with depth_pro model")
...
def main():
    ...
    # Load depth_pro model and transforms
    ...
    for idx1, idx2 in tqdm(pairs, desc="Processing pairs"):
        for idx in (idx1, idx2):
            ...
            # Load image with PIL
            pil_img, _, f_px = depth_pro.load_rgb(img_path)
            image_t = transform(pil_img).to(DEVICE)

            # Run inference using depth_pro
            with torch.no_grad():
